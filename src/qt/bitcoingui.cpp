    ...
    platformStyle(platformStyle)
{
    ...
    QString windowTitle = tr("chips Core") + " - ";
#ifdef ENABLE_WALLET
    ...
    frameBlocksLayout->addStretch();
    frameBlocksLayout->addWidget(labelMegaBlockIcon);
    frameBlocksLayout->addStretch();
    ...
    QTimer* timerMegaBlockIcon = new QTimer(labelMegaBlockIcon);
    connect(timerMegaBlockIcon, SIGNAL(timeout()), this, SLOT(setMegaBlockStatus()));
    timerMegaBlockIcon->start(10000);
    setMegaBlockStatus();
}
...
void BitcoinGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/" + theme + "/about"), tr("&About chips Core"), this);
    aboutAction->setStatusTip(tr("Show information about chips Core"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for chips Core"));
    ...
    openAction->setStatusTip(tr("Open a chips: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the chips Core help message to get a list with possible chips Core command-line options"));
...
void BitcoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    ...
    QString toolTip = tr("chips Core client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to chips network", "", count));
}
...
{
    QString strTitle = tr("chips Core"); // default title
...
void BitcoinGUI::setMegaBlockStatus()
{
    QString theme = GUIUtil::getThemeName();
    if (fMegaBlock) {
        labelMegaBlockIcon->show();
        labelMegaBlockIcon->setPixmap(QIcon(":/icons/" + theme + "/megablock_active").pixmap(STATUSBAR_ICONSIZE, STATUSBAR_ICONSIZE));
        labelMegaBlockIcon->setToolTip(tr("Mega Block is ACTIVE"));
    } else {
        labelMegaBlockIcon->show();
        labelMegaBlockIcon->setPixmap(QIcon(":/icons/" + theme + "/megablock_inactive").pixmap(STATUSBAR_ICONSIZE, STATUSBAR_ICONSIZE));
        labelMegaBlockIcon->setToolTip(tr("Mega Block is not active"));
    }
}
