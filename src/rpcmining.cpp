...
UniValue getmininginfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"next blocks reward\": nnn,             (numeric) The next block reward\n"
            ...
            "  \"mega block\": Active|Not Active             (numeric) The mega block status\n"
            "}\n"
            "\nExamples:\n"
    ...
    obj.push_back(Pair("next block reward", GetBlockSubsidy(pindexPrev->nBits, pindexPrev->nHeight + 1, Params().GetConsensus())));
    ...
    obj.push_back(Pair("mega block", (bool)fMegaBlock ? "Active" : "Not Active"));
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "chips Core is not connected!");

    if(sporkManager.IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)) {
       if (IsInitialBlockDownload())
          throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "chips Core is downloading blocks...");

       if (!masternodeSync.IsSynced())
          throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "chips Core is syncing with network...");
    }
