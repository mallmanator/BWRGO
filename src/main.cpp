...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
	...
	nSubsidy = 20 * COIN;
        ...
        nSubsidy = 10 * COIN;
    } else if (nPrevHeight >= 17280 && nPrevHeight <= 17400) { // Every month there's 
	nSubsidy = 200 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 34560 && nPrevHeight <= 34610) { // Mega blocks : 50 BLOCK
	nSubsidy = 200 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 51840 && nPrevHeight <= 51890) { // For 2 Years
	nSubsidy = 200 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 69120 && nPrevHeight <= 69170) {
        nSubsidy = 200 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 86400 && nPrevHeight <= 86450) {
	nSubsidy = 200 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 103680 && nPrevHeight <= 103730) {
        nSubsidy = 200 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 120960 && nPrevHeight <= 121010) {
	nSubsidy = 200 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 138240 && nPrevHeight <= 138290) {
        nSubsidy = 200 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 155520 && nPrevHeight <= 155570) {
        nSubsidy = 200 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 172800 && nPrevHeight <= 172850) { 
	nSubsidy = 200 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 190080 && nPrevHeight <= 190130) { 
	nSubsidy = 200 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 207360 && nPrevHeight <= 207410) { // 1 YEAR ~ halfed 50% the reward
        nSubsidy = 200 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 224640 && nPrevHeight <= 224690) {
	nSubsidy = 100 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 241920 && nPrevHeight <= 241970) {
        nSubsidy = 100 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 259200 && nPrevHeight <= 259250) {
	nSubsidy = 100 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 276480 && nPrevHeight <= 276530) {
        nSubsidy = 100 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 293760 && nPrevHeight <= 293810) {
        nSubsidy = 100 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 311040 && nPrevHeight <= 311090) {
	nSubsidy = 100 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 328320 && nPrevHeight <= 328370) {
	nSubsidy = 100 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 345600 && nPrevHeight <= 345650) {
        nSubsidy = 100 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 362880 && nPrevHeight <= 362930) {
	nSubsidy = 100 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 380160 && nPrevHeight <= 380210) {
        nSubsidy = 100 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 397440 && nPrevHeight <= 397490) {
	nSubsidy = 100 * COIN; fMegaBlock = true;
    } else if (nPrevHeight >= 414720 && nPrevHeight <= 414770) { // 2 YEAR ~ No Mega block
        nSubsidy = 100 * COIN; fMegaBlock = true;
    } else {
        nSubsidy = 5 * COIN; fMegaBlock = false;
    }
...
{
    {
	if (nHeight < 207360) { // First year 50% MN reward
    	return blockValue * 0.50;
    	}

    return blockValue * 0.40; // 2nd year ~ 40% MN reward
    }
}
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // CHIPS : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(CHIPS): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(CHIPS): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // CHIPS : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(CHIPS): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(CHIPS): spork is off, skipping transaction locking checks\n");
    }
